...
// Get one complete candidate for analysis
const candidate = JSON.parse(
  db.prepare(`SELECT candidate_json FROM candidates ORDER BY id DESC LIMIT 1`).get().candidate_json
);
...
**We actually have:**
\`\`\`javascript
candidate.metrics.liquidityUsd: ${candidate.metrics.liquidityUsd}
candidate.gmgn.liquidity: ${candidate.gmgn?.liquidity}
\`\`\`
...
candidate.metrics = {
  ...
  gmgnTradeFeesSol: ${candidate.metrics.gmgnTradeFeesSol}
}
\`\`\`
...
// Values: ${((candidate.metrics.gmgnTradeFeesSol / candidate.metrics.gmgnTotalFeesSol) * 100).toFixed(1)}%
\`\`\`
...
**We actually have:**
\`\`\`javascript
candidate.createdAtMs: ${candidate.createdAtMs}
\`\`\`
...
// Save to file
fs.writeFileSync(
  'e:\\Falikh\\charon\\DATA_COMPLETENESS_REPORT.md',
  report
);

console.log(report);
console.log('\n\n✅ Report saved to DATA_COMPLETENESS_REPORT.md');
